...
#define CA_EVENT_TLP_RECOVERY 4
#endif
...
#define TCP_ECN_ECT_PERMANENT 4
#endif
#ifndef CA_EVENT_TLP_RECOVERY
#define CA_EVENT_TLP_RECOVERY 4
#endif
...
{
	if (!(tcp_sk(sk)->ecn_flags & TCP_ECN_OK)) return false;
 ...
 */
static u32 bbr_tso_segs(struct sock *sk, unsigned int mss_now, u32 gso_max_size)
{
	u32 segs;
	u64 bytes;

	bytes = READ_ONCE(sk->sk_pacing_rate) >> READ_ONCE(sk->sk_pacing_shift);
	bytes = min_t(u32, bytes, (u64)gso_max_size - 1 - MAX_TCP_HEADER);
			 ...
			 struct bbr_context *ctx)
{
	...
	if (!acked)
	if (bbr->reduce_cwnd) {
    cwnd = max_t(s32, (s32)cwnd - (s32)acked, 4);
    bbr->reduce_cwnd = 0; 
  }
...
/* Calculate the prior_in_flight level that corresponded to excessive loss.
	...
	struct sock *sk, const struct rate_sample *rs, struct bbr_context *ctx)
{
	...
	if (bbr->loss_round_start /* && !rs->is_acking_tlp */) {
...
static void bbr_skb_marked_lost(struct sock *sk, const struct sk_buff *skb)
{
	...
	rs.prior_in_flight = TCP_SKB_CB(skb)->tx.in_flight;
	rs.losses = tp->lost - TCP_SKB_CB(skb)->tx.lost;

	if (rs.losses > 0)
		bbr->inflight_hi = bbr_inflight_hi_from_lost_skb(sk, &rs, skb);
#endif
}
...
static int bbr_get_phase(struct bbr *bbr)
{
	switch (bbr->mode) {
	case BBR_STARTUP:
			    ...
			    union tcp_cc_info *info)
{
  if (ext & (1 << (INET_DIAG_BBRINFO - 1))) {
		...
		bbr_info->bbr_bw_lo = (u32)bbr_bw(sk);
		bbr_info->bbr_bw_hi = (u32)(bbr_bw(sk) >> 32);
